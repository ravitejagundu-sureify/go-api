...
func RegisterHandle(w http.ResponseWriter, r *http.Request) {
	//this page handles all registration requests
	...
	if err != nil {
		...
		_, err = db.Exec(query, mail_id, username, password)
...
func LoginHandler(w http.ResponseWriter, r *http.Request) {
	// fmt.Fprintf(w, "this func handles all the login requests")
	session, _ := store.Get(r, "sessions")
	_, ok := session.Values["username"]
...
func UpdateHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Fatal("Database connection problem")
	}
	query := `SELECT mail_id FROM Details WHERE mail_id =?`

	row := db.QueryRow(query, data.Mailid)
